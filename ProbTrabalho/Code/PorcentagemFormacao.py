...
porcentagem_vitorias_sorted_filtrada = porcentagem_vitorias.sort_values(ascending=False)

# Criar um DataFrame com as porcentagens e formar variáveis dummy
porcentagem_vitorias_df_filtrada = porcentagem_vitorias_sorted_filtrada.reset_index()
porcentagem_vitorias_df_filtrada.columns = ['formacao', 'porcentagem']

# Plotar o gráfico com as formações no eixo X e as porcentagens no eixo Y
plt.figure(figsize=(12, 6))
plt.bar(porcentagem_vitorias_df_filtrada['formacao'], porcentagem_vitorias_df_filtrada['porcentagem'], edgecolor='black')
plt.title('Porcentagem de Vitórias por Formação (Com >= 38 Partidas)')
